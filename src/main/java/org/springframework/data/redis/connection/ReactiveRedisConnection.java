...
public interface ReactiveRedisConnection extends Closeable {
	...
	static interface Command {
		...
		public static <T extends Command> Builder<T> create(Class<T> type) {
			return new CommandBuilder<T>(type);
		}

		public interface Builder<T extends Command> extends Consumer<Object> {
		...
		class CommandBuilder<T extends Command> implements Builder<T> {
			...
			Supplier<Object[]> arguments = () -> (argumentList.size() == 1 && argumentList.get(0) instanceof Supplier)
					? ObjectUtils.toObjectArray(((Supplier<?>) argumentList.get(0)).get()) : argumentList.toArray();
